...
# --- MLB run expectancy (base-out) reference (FanGraphs "Run Expectancy Matrix Reloaded") ---
# Values are runs expected from the given base/out state until end of inning.
# Using 2021–2024 MLB average as a stable default (good for cross-league context).
MLB_RE_2021_24 = {
    "---": {0: 0.50, 1: 0.27, 2: 0.10},
    "1--": {0: 0.90, 1: 0.54, 2: 0.23},
    "-2-": {0: 1.16, 1: 0.72, 2: 0.34},
    "--3": {0: 1.43, 1: 0.98, 2: 0.37},
    "12-": {0: 1.54, 1: 0.94, 2: 0.45},
    "1-3": {0: 1.84, 1: 1.19, 2: 0.49},
    "-23": {0: 1.98, 1: 1.39, 2: 0.56},
    "123": {0: 2.45, 1: 1.55, 2: 0.72},
}
_MLB_RE_FLAT_2021_24 = {f"{b}_{o}": v for b, outs in MLB_RE_2021_24.items() for o, v in outs.items()}

def _truthy_runner(x) -> bool:
    if x is None:
        return False
    s = str(x).strip()
...
def _base_state_from_cols(r1, r2, r3) -> str:
    return ("1" if _truthy_runner(r1) else "-") + ("2" if _truthy_runner(r2) else "-") + ("3" if _truthy_runner(r3) else "-")

def _map_mlb_re(base_state: pd.Series, outs: pd.Series) -> pd.Series:
    # base_state like '1--', outs in {0,1,2}
    key = base_state.astype("string").str.strip().replace({"": pd.NA}) + "_" + outs.astype("Int64").astype("string")
    return key.map(_MLB_RE_FLAT_2021_24)
...
STUFF_TILE_CLIP = 15.0  # +/- points vs league baseline used for Stuff+ tile coloring
# ------------------------------
...
# ──────────────────────────────────────────────────────────────
# STUFF+ / SHAPE (self-contained)
#   Goal: add an outcome-free *evaluation* layer by predicting RunValue
#   using ONLY raw pitch characteristics (velo/spin/movement/release/etc.).
#
#   This is the same basic idea as public "Stuff+" models: use physics inputs,
#   train to run value, and then scale so league avg ~ 100. The point is NOT
#   to replace RV; it's to show "shape quality" vs "results".
# ──────────────────────────────────────────────────────────────

# Abbrev sets for pitch-family bucketing
_PT_FASTBALL = {"FF","FA","SI","FT","FC","CT"}
_PT_BREAKING = {"SL","SW","CB","KC","CU","SC","EP"}
_PT_OFFSPEED = {"CH","SF","FS","FO","KN"}

_STUFF_BASE_FEATURES = [
    "Velo",
    "SPIN_RATE",
    "INDUCED_VERT_BREAK",
    "HORZ_BREAK",
...
def _pitch_code(series: pd.Series) -> pd.Series:
    # keep abbreviations; map full names to abbrev via pitch_abbr()
    return series.apply(lambda x: pitch_abbr(x) if pd.notna(x) else pd.NA).astype("string")

def _pitch_family(code: str) -> str:
    if not isinstance(code, str):
        return "UNK"
    if code in _PT_FASTBALL: return "FB"
    if code in _PT_BREAKING: return "BR"
    if code in _PT_OFFSPEED: return "OS"
    return "UNK"

def _primary_fastball_baseline(df: pd.DataFrame) -> pd.DataFrame:
    """
    Per-pitcher baseline fastball metrics to compute differentials for secondaries.
    If a pitcher has no FB in the file, we fall back to league FB averages.
    """
    if "Pitcher" not in df.columns:
        # no pitcher column? just use league baseline for diffs
        base = df.copy()
        for c in _STUFF_BASE_FEATURES:
            base[f"FB_{c}"] = df[c].mean()
        return base

    fb_mask = df["PT_CODE"].isin(list(_PT_FASTBALL))
    lg_fb = df.loc[fb_mask, _STUFF_BASE_FEATURES].mean(numeric_only=True)

    by_p = (df.loc[fb_mask]
              .groupby("Pitcher")[_STUFF_BASE_FEATURES]
              .mean(numeric_only=True)
              .rename(columns={c: f"FB_{c}" for c in _STUFF_BASE_FEATURES})
           )

    out = df.join(by_p, on="Pitcher")
    # pitcher with no FB → league baseline
    for c in _STUFF_BASE_FEATURES:
        out[f"FB_{c}"] = out[f"FB_{c}"].fillna(lg_fb.get(c, np.nan))
    return out

def _ridge_fit(X: np.ndarray, y: np.ndarray, lam: float = 10.0):
    """
    Simple ridge regression w/ intercept using closed form.
    Returns (beta, mu, sig) where X is standardized by mu/sig.
    """
...
def _ridge_predict(beta, mu, sig, X: np.ndarray):
    Xs = (X - mu) / sig
    X1 = np.concatenate([np.ones((Xs.shape[0], 1)), Xs], axis=1)
    return X1 @ beta

def add_stuff_plus(df: pd.DataFrame):
    """
    Adds:
      - xRV_Stuff   : predicted RunValue from *shape-only* inputs
      - StuffPlus   : scaled (100 = league avg within pitch-family)
      - PT_CODE     : pitch abbrev used for family mapping

    Requirements:
      - RunValue column must exist (to train)
      - At least some pitch metrics from _STUFF_BASE_FEATURES
    """
    if df is None or df.empty:
        return df, " | Stuff+: no data"

    # Make sure PitchType + RunValue exist
    if "PitchType" not in df.columns or "RunValue" not in df.columns:
        df["xRV_Stuff"] = np.nan
        df["StuffPlus"] = np.nan
        df["PT_CODE"] = pd.NA
        return df, " | Stuff+: missing PitchType/RunValue"

    d = df.copy()
    d["PT_CODE"] = _pitch_code(d["PitchType"])
    d["PT_FAM"] = d["PT_CODE"].apply(_pitch_family)

    # numeric coercion for features + RunValue
    d = _to_num(d, _STUFF_BASE_FEATURES + ["RunValue"])

    # build FB baselines + differentials
    d = _primary_fastball_baseline(d)
    for c in _STUFF_BASE_FEATURES:
        d[f"d{c}"] = d[c] - d[f"FB_{c}"]

    feat_cols = _STUFF_BASE_FEATURES + [f"d{c}" for c in _STUFF_BASE_FEATURES]

    # train separate ridge per pitch family (FB/BR/OS). UNK → no prediction.
    d["xRV_Stuff"] = np.nan

    trained = 0
    for fam in ["FB", "BR", "OS"]:
        mask = (d["PT_FAM"] == fam) & d["RunValue"].notna()
        # require complete features
        mask = mask & d[feat_cols].notna().all(axis=1)
        n = int(mask.sum())
        if n < 300:
            # too few rows = unstable; skip
            continue

        X = d.loc[mask, feat_cols].to_numpy(dtype=float)
        y = d.loc[mask, "RunValue"].to_numpy(dtype=float)

        beta, mu, sig = _ridge_fit(X, y, lam=12.0)

        # predict for rows in this family with complete features
        mask_pred = (d["PT_FAM"] == fam) & d[feat_cols].notna().all(axis=1)
        Xp = d.loc[mask_pred, feat_cols].to_numpy(dtype=float)
        d.loc[mask_pred, "xRV_Stuff"] = _ridge_predict(beta, mu, sig, Xp)
        trained += 1

    # scale to StuffPlus (higher = better)
    d["StuffPlus"] = np.nan
    for fam in ["FB", "BR", "OS"]:
        fam_mask = (d["PT_FAM"] == fam) & d["xRV_Stuff"].notna()
        if not fam_mask.any():
            continue
        # pitcher-friendly direction: lower RV is better → flip sign
        val = (-d.loc[fam_mask, "xRV_Stuff"]).astype(float)
        mu = float(val.mean())
        sd = float(val.std(ddof=0)) if float(val.std(ddof=0)) > 1e-9 else 1.0
        d.loc[fam_mask, "StuffPlus"] = 100.0 + 10.0 * ((val - mu) / sd)

    # keep it sane for tiles
    d["StuffPlus"] = d["StuffPlus"].clip(40, 160)

    msg = ""
    if trained == 0:
        msg = " | Stuff+: not enough data to train (need ~300+ pitches per family w/ metrics)"
    else:
        msg = f" | Stuff+: enabled ({trained}/3 families trained)"
    return d, msg
...
def load_pitchlog(data_bytes):
    ...
    # Always keep league RV as the baseline
    df["RunValue_LG"] = df["RunValue"]

    # Optional: compute MLB-context RV using MLB base/out RE (requires before+after base/out state columns)
    df["RunValue_MLB"] = np.nan
    df["RE_before_MLB"] = np.nan
    df["RE_after_MLB"]  = np.nan

    def _find_any_ci(*cands):
        want = {c.lower() for c in cands}
        for c in df.columns:
            if c.lower() in want:
                return c
        return None

    bs_b = _find_any_ci("base_state_before","bases_before","base_state_pre","base_state")
    bs_a = _find_any_ci("base_state_after","bases_after","base_state_post","base_state_end")

    outs_b = _find_any_ci("outs_before","outs_pre","outs_when_up","outs")
    outs_a = _find_any_ci("outs_after","outs_post","outs_end","outs_after_play")

    if bs_b and outs_b and bs_a and outs_a:
        bstate_before = df[bs_b].astype("string").str.strip()
        bstate_after  = df[bs_a].astype("string").str.strip()
        ob = pd.to_numeric(df[outs_b], errors="coerce").astype("Int64")
        oa = pd.to_numeric(df[outs_a], errors="coerce").astype("Int64")
        df["RE_before_MLB"] = _map_mlb_re(bstate_before, ob)
        df["RE_after_MLB"]  = _map_mlb_re(bstate_after, oa)
    else:
        # Try to build base state from runner-id columns (Statcast-ish)
        r1_b = _find_any_ci("on_1b","runner_on_1b","r1","first_base","on_first")
        r2_b = _find_any_ci("on_2b","runner_on_2b","r2","second_base","on_second")
        r3_b = _find_any_ci("on_3b","runner_on_3b","r3","third_base","on_third")
        r1_a = _find_any_ci("on_1b_after","runner_on_1b_after","r1_after","on_first_after")
        r2_a = _find_any_ci("on_2b_after","runner_on_2b_after","r2_after","on_second_after")
        r3_a = _find_any_ci("on_3b_after","runner_on_3b_after","r3_after","on_third_after")

        if outs_b and outs_a and r1_b and r2_b and r3_b and r1_a and r2_a and r3_a:
            bstate_before = df.apply(lambda r: _base_state_from_cols(r[r1_b], r[r2_b], r[r3_b]), axis=1)
            bstate_after  = df.apply(lambda r: _base_state_from_cols(r[r1_a], r[r2_a], r[r3_a]), axis=1)
            ob = pd.to_numeric(df[outs_b], errors="coerce").astype("Int64")
            oa = pd.to_numeric(df[outs_a], errors="coerce").astype("Int64")
            df["RE_before_MLB"] = _map_mlb_re(pd.Series(bstate_before, index=df.index), ob)
            df["RE_after_MLB"]  = _map_mlb_re(pd.Series(bstate_after, index=df.index), oa)

    if df["RE_before_MLB"].notna().any() and df["RE_after_MLB"].notna().any():
        runs = pd.to_numeric(df.get("runs_scored_on_pitch", 0), errors="coerce").fillna(0)
        df["RunValue_MLB"] = runs + df["RE_after_MLB"] - df["RE_before_MLB"]
...
app.layout = html.Div(style={"fontFamily": "Arial",
    ...
    html.Div(style={"display": "flex", "gap": "10px",
        ...
        html.Div(
            ...
            children=[
                html.Div("RV context", style={"fontSize": "12px", "color": "#444", "marginBottom": "2px"}),
                dcc.RadioItems(
                    id="rv-context",
                    options=[
                        {"label": "League", "value": "league"},
                        {"label": "MLB (RE 2021–24)", "value": "mlb"},
                    ],
                    value="league",
                    inline=True,
                    style={"fontSize": "12px"},
    ...
    html.Div(
        style={"textAlign":"center","fontSize":"13px","color":"#555","margin":"0 0 6px 0"},
        children="Stuff+ (shape-only): predicted RV from velo/spin/movement/release (100 = league avg)",
    ),
    html.Div(
        id="pitch-stuff-tiles",
        style={
            "display": "flex",
            "gap": "10px",
            "justifyContent": "center",
...
# ---------- FIX filtered subset (drives everything downstream) ---------- #
@app.callback(
    ...
    Input("rv-context",  "value"),
)
def make_subset(raw_json, pitcher, dates, hand, ptype, rv_context):
    if not raw_json:
        return None
    ...
    if ptype:
        if isinstance(ptype, (list, tuple, set)):
            df = df[df["PitchType"].isin(list(ptype))]
        else:
            df = df[df["PitchType"] == ptype]

    # Apply RV context (defaults to league if MLB RV is unavailable)
    if "RunValue_LG" not in df.columns and "RunValue" in df.columns:
        df["RunValue_LG"] = df["RunValue"]

    if rv_context == "mlb" and "RunValue_MLB" in df.columns and df["RunValue_MLB"].notna().any():
        df["RunValue"] = df["RunValue_MLB"]
    elif "RunValue_LG" in df.columns:
        df["RunValue"] = df["RunValue_LG"]
...
def handle_upload(contents, filename):
    ...
    # Stuff+/Shape (raw-metric evaluation layer)
    stuff_msg = ""
    try:
        df, stuff_msg = add_stuff_plus(df)
    except Exception as e:
        # Keep dashboard alive if a file is missing pitch metrics
        df["xRV_Stuff"] = np.nan
        df["StuffPlus"] = np.nan
        df["PT_CODE"] = pd.NA
        stuff_msg = f" | Stuff+: error ({type(e).__name__})"
    ...
    status = f"Loaded {len(df):,} pitches from {filename}{stuff_msg}"
...
def update_pitch_rv_tiles(raw_json, sub_json):
    if not raw_json or not sub_json:
        return []

    raw = pd.read_json(raw_json, orient="split")
    sub = pd.read_json(sub_json, orient="split")

    # Tagged-only for pitch-type tables (untagged still counts elsewhere)
    raw_t = raw.dropna(subset=["PitchType"]).copy()
    sub_t = sub.dropna(subset=["PitchType"]).copy()

    if sub_t.empty:
        return []

    # League baseline by pitch type (mean RV/pitch)
    lg_mean = raw_t.groupby("PitchType")["RunValue"].mean()

    grp = (
        sub_t.groupby("PitchType")
            .agg(
                N=("PitchType", "size"),
                RV_sum=("RunValue", "sum"),
    ...
    tot = grp["N"].sum()
    grp["Usage"] = grp["N"] / tot
    grp["LG_avg"] = grp["PitchType"].map(lg_mean)
    grp["Diff"] = grp["RV_avg"] - grp["LG_avg"]

    # Sort by usage (default view coaches expect)
    grp = grp.sort_values("Usage", ascending=False)
    ...
    for _, r in grp.iterrows():
        ...
        # Negative RV is good for the pitcher (prevents runs)
        border = "rgba(0,0,0,0.25)"
        if pd.notna(diff):
            border = "rgba(0,150,0,0.55)" if diff < 0 else "rgba(200,0,0,0.55)"

        tiles.append(
            html.Div(
                children=[
                    html.Div([
                        html.Span(pt, style={"fontWeight": 700, "fontSize": "16px"}),
                        html.Span(f"{usage:.0%} usage", style={"fontSize": "12px", "color": "#666"}),
                    ], style={"display": "flex", "justifyContent": "space-between"}),

                    html.Div(f"{rv_avg:+.3f} RV / pitch", style={"fontSize": "18px", "fontWeight": 700, "marginTop": "6px"}),

                    html.Div(
                        [
                            html.Span(f"League: {lg_avg:+.3f}" if pd.notna(lg_avg) else "League: —", style={"fontSize": "12px", "color": "#666"}),
                            html.Span(f"Δ {diff:+.3f}" if pd.notna(diff) else "Δ —", style={"fontSize": "12px", "color": "#666"}),
                        ],
...
@app.callback(
    Output("pitch-stuff-tiles", "children"),
    Input("raw-data",      "data"),
    Input("filtered-data", "data"),
)
def update_pitch_stuff_tiles(raw_json, sub_json):
    if not raw_json or not sub_json:
        return []
    ...
    if sub_t.empty or "StuffPlus" not in sub_t.columns:
        return []

    # League baseline by pitch type (mean StuffPlus)
    lg_mean = raw_t.groupby("PitchType")["StuffPlus"].mean() if "StuffPlus" in raw_t.columns else None

    grp = (
        sub_t.groupby("PitchType")
            .agg(
                N=("PitchType", "size"),
                Stuff=("StuffPlus", "mean"),
                Velo=("Velo", "mean") if "Velo" in sub_t.columns else ("PitchType", "size"),
            )
            .reset_index()
    )
    if "Velo" not in grp.columns:
        grp = grp.drop(columns=["Velo"])

    tot = grp["N"].sum()
    grp["Usage"] = grp["N"] / tot
    if lg_mean is not None:
        grp["LG_Stuff"] = grp["PitchType"].map(lg_mean)
        grp["Diff"] = grp["Stuff"] - grp["LG_Stuff"]
    else:
        grp["LG_Stuff"] = np.nan
        grp["Diff"] = np.nan
        ...
        tiles.append(
            html.Div(
                children=[
                    ...
                    html.Div(f"{stuff:.1f} Stuff+", style={"fontSize": "18px", "fontWeight": 700, "marginTop": "6px"}),

                    html.Div(
                        [
                            html.Span(f"League: {lg_stuff:.1f}" if pd.notna(lg_stuff) else "League: —", style={"fontSize": "12px", "color": "#666"}),
                            html.Span(f"Δ {diff:+.1f}" if pd.notna(diff) else "Δ —", style={"fontSize": "12px", "color": "#666"}),
                        ],
                        style={"display": "flex", "justifyContent": "space-between", "marginTop": "4px"},
                    ),
                ],
...
def update_custom_scatter(sub_json, x_var, y_var, color_var, ptype_filter, agg_mode):
    ...
    # Expected vs actual movement ring (mean ±1 SD for the currently displayed subset)
    try:
        if x_var == "HORZ_BREAK" and y_var == "INDUCED_VERT_BREAK" and len(df) >= 15:
            mx = float(pd.to_numeric(df.get("HORZ_BREAK"), errors="coerce").mean())
            my = float(pd.to_numeric(df.get("INDUCED_VERT_BREAK"), errors="coerce").mean())
            sx = float(pd.to_numeric(df.get("HORZ_BREAK"), errors="coerce").std())
            sy = float(pd.to_numeric(df.get("INDUCED_VERT_BREAK"), errors="coerce").std())
            rr = float(np.nanmean([sx, sy]))
            if np.isfinite(rr) and rr > 0:
                rr = max(min(rr, 8.0), 0.5)
                fig.add_shape(
                    type="circle",
                    xref="x", yref="y",
                    x0=mx-rr, x1=mx+rr,
                ...
                fig.add_annotation(
                    x=mx, y=my+rr,
                    text="Expected movement (mean ±1 SD)",
                    showarrow=False,
                    yshift=12,
