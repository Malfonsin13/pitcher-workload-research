# -*- coding: utf-8 -*-
"""Usage-Sequencing Dashboard

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1n3onSF1RfSUvaGR7Wf6En_Ysij84kW-F
"""
...
def update_heatmaps(sub_json, ptype, mode, custom_sel):
    ...
    # Clean PitchType once
    df = df.copy()
    df["_PT"] = _clean_pt_series(df["PitchType"])

    # 3% keep-set computed vs tagged-only PitchType (overall in this filtered dataset)
    keep = _keep_pitchtypes(df, threshold=0.03)
    ...
    if ptype:
        ...
        # Respect "overall usage" threshold: don't show <3% pitch even if selected
        if p_clean not in keep:
            blank = go.Figure().update_layout(
                title=f"No data after 3% usage threshold (selected {p_clean} is <3%)",
                xaxis_visible=False, yaxis_visible=False
            )
            return [blank] * 5
        df = df[df["_PT"] == p_clean].copy()
    ...
    for _, ab in _group_by_ab(df):
        ...
        ab["_PT"] = _clean_pt_series(ab["PitchType"])
        ab["_valid"] = ab["_PT"].isin(keep)

        # Compute prev on the ORIGINAL sequence (no bridging)
        ab["prev"] = ab["_PT"].shift(1)
        ab["prev_valid"] = ab["_valid"].shift(1)

        # Keep only true adjacent transitions where both prev and current are valid
        ab_t = ab[ab["_valid"] & ab["prev_valid"]].copy()
        if ab_t.empty:
            continue

        ab_t["PitchType"] = ab_t["_PT"]
        rows.append(ab_t[["prev", "PitchType", "csw"]])
    ...
    MIN_OVERALL_TRANS = 3.0  # % of ALL transitions for a given next pitch
    ...
    # Usage % by bucket
    # Use cleaned pitch type for bucket heatmaps
    df_exp["_PT"] = _clean_pt_series(df_exp["PitchType"])

    # counts over TAGGED pitch types only (NaNs drop out automatically)
    usage_cnt_all = (df_exp.dropna(subset=["_PT"])
                          .groupby(["BucketList", "_PT"])
                          .size()
                          .unstack(fill_value=0)
                          .reindex(ROW_ORDER, fill_value=0))

    usage_pct_all = (usage_cnt_all
                    .div(usage_cnt_all.sum(axis=1).replace(0, np.nan), axis=0)
                    .mul(100)
                    .fillna(0))

    # DISPLAY only pitch types meeting 3% overall usage
    disp_cols = [c for c in usage_cnt_all.columns if c in keep]
    usage_cnt = usage_cnt_all[disp_cols]
    usage_pct = usage_pct_all[disp_cols]
    ...
    # CSW % by bucket
    csw_succ_b_all = (df_exp.dropna(subset=["_PT"])
                          .groupby(["BucketList", "_PT"])["csw"]
                          .sum()
                          .unstack(fill_value=0)
                          .reindex(ROW_ORDER, fill_value=0))

    csw_succ_b = csw_succ_b_all[disp_cols]
    ...
    # 1) Exclude pitches with < 3% OVERALL usage from any consideration
    MIN_OVERALL_USAGE = 3.0  # percent of total pitches across all buckets
    ...
    csw_bucket_fig.update_layout(
        title=dict(
            text=(
                "CSW % by Bucket (green = high CSW & under-used; red = low CSW & over-used)"
                "<br><sup>Color = product of CSW and usage z-scores; "
                "pitches &lt;5% overall usage shown neutral</sup>"
            )
        ),
        margin=dict(l=0, r=0, t=40, b=0),
    )
    ...
    velo_val = (
        df_exp.dropna(subset=["_PT"])
          .groupby(["BucketList", "_PT"])["Velo"]
          .mean()
          .unstack()
          .reindex(index=ROW_ORDER, columns=disp_cols)
    )
